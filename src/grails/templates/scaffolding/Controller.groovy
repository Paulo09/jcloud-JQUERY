<%=packageName ? "package ${packageName}\n\n" : ''%>class ${className}Controller {
	...
	def login = {
		...
		if(usuario && usuario.permissao=="ADMIN")
		{
		 ...
		 print "Logado"
		 redirect(url:"/sergio/menu.gsp") 
		}
		if(usuario && usuario.permissao=="USUARIO")
		{
		 session["senha"] = senhaUsuario
		 session["usuario"] = loginUsuario
		 //redirect(url:"/spteste/criarTeste/list") 
		}
		if(!usuario){flash.message = "${className} nao cadastrado \${params.id}"}
	}
	def sair={
	  session["senha"]= null
      session["usuario"] = null
	  session.invalidate()
	  redirect(url:"/spteste")
	}
	
	def camera = {
	   if(params.id){
	   ${className} objCamera = ${className}.findById(params.id)
	   if(objCamera){
	      response.outputStream << objCamera.foto
	   }
	 }
	}
